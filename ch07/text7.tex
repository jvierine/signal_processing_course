...
\newthought{A Fourier series represents a periodic signal in the frequency domain as a sum of complex sinusoids}. How do we ensure that the resulting signal is periodic? We do so by construction: every sinusoidal component is chosen to repeat after the same period $T$. Equivalently, every angular frequency is an integer multiple of a common base angular frequency $\omega$:
\begin{equation}
  x(t) = \sum_{k=1}^N c_k e^{i \omega_k t} = \sum_{k=1}^N c_k e^{i \ell_k \omega t} \,\,.
  \label{eq:fourier_series_0}
\end{equation}
Here $\ell_k \in \mathbb{Z}$ is integer valued, so each angular frequency is an integer multiple of the positive angular base frequency $\omega \in \mathbb{R}_{>0}$:
\begin{equation}
  \omega_k = \ell_k\omega \,\,.
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \begin{axis}[width=1.5\textwidth, height=30em,
        ...
        \addplot[domain=0:4.18879,samples=600,color=blue] {cos(deg(9*x))-cos(deg(15*x))};
        \addplot[domain=0:4.18879,samples=600,color=red] {sin(deg(9*x))-sin(deg(15*x))};

        \node at (axis cs:1.0472,2.5) {$\displaystyle{T}$};
        \addplot [dimen] plot coordinates {(0,2.3) (2.0944,2.3)};
        \node at (axis cs:3.14159,2.5) {$\displaystyle{T}$};
        \addplot [dimen] plot coordinates {(2.0944,2.3) (4.18879,2.3)};
    ...
    \end{tikzpicture}
  \end{center}
  \caption{Periodic function $e^{i9t} - e^{i15t}$.
...
\begin{equation}
  x(t) = c_1 e^{i \omega_1 t} + c_2 e^{i \omega_2 t} = e^{i9t} - e^{i15t} \,\,.
\end{equation}
...
This signal has two unique angular frequencies: $\omega_1 = 9$
and $\omega_2 = 15$.  Let's use Euclid's algorithm\sidenote{The
...
numbers: $(9,15)$. Then subtract the smallest number from
the largest number $15-9=6$ and replace the larger
number with the remainder: $(9,6)$.  We repeat this process
...
\begin{align*}
  ...
  (6-3,3)    & = (3,3).
\end{align*}
And we thus find that the fundamental angular frequency is $\omega=3$ and, using
Equation \ref{eq:fundamental_period}, we find that $T = 2\pi/3$.

For more than two angular frequencies, Euclid's algorithm is applied recursively:
\begin{equation}
  \gcd(\omega_1,\omega_2,\ldots,\omega_N)
  = \gcd\!\left(\gcd(\omega_1,\omega_2),\omega_3,\ldots,\omega_N\right) \,\,.
\end{equation}
If, at any point, a pair of frequencies is not commensurable, we can stop:
...
\end{equation}
Here, we express the fundamental angular frequency as a function of the
fundamental period, $\omega=2\pi/T$.
...
solutions \cite{fourier1822theorie,GRATTANGUINNESS2005354}. Perhaps the most revolutionary outcome of his study was
that a wide range of functions can be represented as a sum of
elementary sinusoidal functions. The Fourier representation is one of
the most important, if not the most important, tools for analyzing
...
\begin{marginfigure}[-8cm]
  ...
  \caption{A Fourier series representation $x_{7}(t)=\frac{1}{T}\sum_{k=-7}^{7} e^{i\frac{2\pi}{T}k t}$ of a Dirac comb signal $x(t)=\sum_{k=-\infty}^{\infty}\delta(t-k T)$
    ...
    \href{https://juha.no/signal/dirac-comb/}{Interactive demonstration.}}
  \label{fig:fs_fig0}
\end{marginfigure}
...
\newthought{A truncated Fourier series oscillates near a discontinuity}. Consider the periodic extension of a unit step,
\begin{equation}
  u_T(t)=
  \begin{cases}
    0, & -T/2<t<0,\\
  ...
  \qquad u_T(t+T)=u_T(t) \,\,.
\end{equation}
The untapered Fourier series coefficients are
\begin{equation}
  c_k=
  \begin{cases}
    \dfrac{1}{2}, & k=0,\\[4pt]
    \dfrac{1-(-1)^k}{i2\pi k}, & k\ne 0.
  \end{cases}
  \label{eq:gibbs-untapered-coefficients}
\end{equation}
If $N$ odd harmonics are retained and $K=2N-1$, the partial sum is
\begin{equation}
  x_N(t)=\sum_{k=-K}^{K}c_k e^{i2\pi kt/T}
  =\frac{1}{2}+\frac{2}{\pi}\sum_{m=0}^{N-1}
  \frac{\sin\!\left((2m+1)2\pi t/T\right)}{2m+1} \,\,.
  \label{eq:gibbs-step-series}
\end{equation}
At the discontinuity the series converges to the midpoint of the jump. Adding
more harmonics makes the oscillating region narrower, but the overshoot
approaches
$\operatorname{Si}(\pi)/\pi-1/2\approx 0.08949$, or $8.95\%$ of the jump,
where $\operatorname{Si}(x)=\int_0^x \sin(u)/u\,du$. This is the
\emph{Gibbs phenomenon}: convergence is pointwise but not uniform, and
analogous ringing occurs near sharp transitions in time or frequency.

Gibbs discussed the phenomenon in two \emph{Nature} letters prompted by
Michelson's harmonic-analyzer observations
\cite{michelson1898fourier}\cite{gibbs1898fourier}\cite{gibbs1899fourier}.

The Fourier coefficients can be tapered to suppress the oscillations. A continuous interpolation between untapered coefficients and Fej\'er weighting is
\begin{align}
  \widetilde{c}_k^{(\alpha)} &= w_{k,\alpha}c_k \,\,,\label{eq:gibbs-tapered-coefficients}\\
  w_{k,\alpha} &= (1-\alpha)+\alpha\left(1-\frac{|k|}{K+1}\right),
  \qquad |k|\le K,\quad 0\le\alpha\le 1 \,\,.
\end{align}
Here $\alpha=0$ gives the ordinary untapered coefficients, while $\alpha=1$ gives full Fej\'er tapering \cite{fejer1900fonctions}. Increasing the taper suppresses the oscillations, but also makes the transition across the step wider.

The same Fourier trade-off appears in radio antennas. The far-field radiation pattern is a spatial Fourier transform of the current distribution across an aperture. Tapering the aperture or array coefficients reduces sidelobes, but widens the main beam \cite{dolph1946current,taylor1955line}. \href{https://juha.no/signal/gibbs-phenomenon/}{The interactive demonstration} shows the tapered and untapered partial sums.
...
\emph{\index{low-pass filtering}{low-pass filtering}} operation would reduce the amplitudes of high
...
\end{align}
In this form, the equation is a \emph{\index{Riemann sum approximation}{Riemann sum approximation}} of an integral of a continuous function $\hat{x}_{T}(\omega)e^{-i\omega t}$.
\begin{figure}
  \begin{center}
    \begin{tikzpicture}
      \begin{axis}[width=13cm,height=6cm,ymin=-0,xmin=-3,ymax=1.8,xmax=3,  yticklabels={,,},
        ...
        \addplot+[ycomb] plot coordinates {(0.5,0.5) (1.0,0.3) (1.5,1.0) (2,0.9)};
        ...
        \addplot[dimen]  plot coordinates {(0.0,0.35) (0.5,0.35)};
        \addplot[mark=none,color=gray] plot coordinates {(-2.25,0.5) (-1.75,0.5) (-1.75,0.6) (-1.25,0.6) (-1.25,0.8) (-.75,0.8) (-.75,1) (-.25,1) (-.25,1.4) (.25,1.4) (.25,0.5) (.75,0.5) (.75,0.3) (1.25,0.3) (1.25,1.0) (1.75,1.0) (1.75,0.9) (2.25,0.9)} ;

        \addplot[mark=none,color=black,hobby] plot coordinates {(-2.5,0.4) (-2.,0.5) (-1.5,0.6) (-1,0.8) (-.5,1) (0,1.4) (.5,.5) (1,0.3) (1.5,1.0) (2,0.9) (2.5,0.8) } ;

        \node at (axis cs:0.0,1.5) [above right, font={\footnotesize}]{$c_0$};
        \node at (axis cs:0.5,0.6) [above, font={\footnotesize}]{$c_1$};
        \node at (axis cs:1.0,0.4) [above, font={\footnotesize}]{$c_2$};
        \node at (axis cs:1.5,1.1) [above, font={\footnotesize}]{$c_3$};
        \node at (axis cs:2.0,1.0) [above, font={\footnotesize}]{$c_4$};
