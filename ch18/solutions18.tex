...
\lstinputlisting[language=Python, caption=Code to plot the impulse response function,label=code18_2,linerange={0-18}]{ch18/code/ex18_2.py}
...
$$Y_{2}(z)=z^{-n_{0}}\mathcal{H}(z)=z^{-2}\mathcal{H}(z)=z^{-1}+z^{-2}+3z^{-3}-0.5z^{-4}+4z^{-12},$$
...
\lstinputlisting[language=Python,caption=Script to plot the magnitude response,label=sol_to_c,linerange={0-27}]{ch18/code/ex18_3.py}
...
\lstinputlisting[language=Python,caption=Code to plot the magnitude response,label=z:code:ex4,linerange={0-26}]{ch18/code/ex18_4.py}
