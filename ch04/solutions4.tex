...
\end{align*}
In the last step the two sums were combined into one and the index was renamed to $k$. From this we can see that the output are equal, so the system is linear. 
...
\lstinputlisting[language=Python,caption={Solution for Exercise 4.8},label=ex4.8]{ch04/code/ex8.py}
...
\lstinputlisting[language=Python,caption={Solution for Exercise 4.9},label=ex4.9]{ch04/code/ex9.py}
