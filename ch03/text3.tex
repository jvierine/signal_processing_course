...
\begin{marginfigure}
  ...
  \begin{tikzpicture}
   ...
   \begin{axis}[axis equal, ymin=-0.3,xmin=-0.3,ymax=1.8,xmax=1.8,  ticks=none,
          xlabel=$\mathrm{Re}$,
        ...
        \addplot [black, mark = *] coordinates {( {1.5*cos(65)}, {1.5*sin(65)} )}  node[anchor=south west]{$z_3 = | z_1 | |z_2|  e^{i (\theta_1 + \theta_2)}$};
...
\end{center}
  \caption{Multiplication of two complex numbers.}
  \label{fig:comp_mult}
\end{marginfigure}
