...
function drawPhasor() {
  ...
  for (let i = 0; i <= 160; i += 1) {
    ...
    circle.push(...map(Math.cos(a), Math.sin(a)));
  }
  phasorRenderer.draw(circle, COLORS.softTeal);

  const initial = map(Math.cos(state.phase), Math.sin(state.phase));
...
function drawSignal() {
  ...
  for (let i = 0; i <= 600; i += 1) {
    ...
    const x = clipX(t, 0, 10, left, right);
    real.push(x, clipY(Math.cos(theta), -1.2, 1.2, bottom, top));
    imaginary.push(x, clipY(Math.sin(theta), -1.2, 1.2, bottom, top));
  }
  ...
  const theta = state.omega * state.time + state.phase;
  const realY = clipY(Math.cos(theta), -1.2, 1.2, bottom, top);
  const imaginaryY = clipY(Math.sin(theta), -1.2, 1.2, bottom, top);
...
function updateEquation() {
  const theta = state.omega * state.time + state.phase;
  els.equation.innerHTML = `\\(\\theta=${signed(state.omega)}(${state.time.toFixed(2)})${signed(state.phase)}=${signed(theta)}\\text{ rad}\\)`;
...
function updateUI(includeEquation = true) {
  ...
  const imaginary = Math.sin(theta);
