...
long ideal filters and truncate these to get a filter with finite length. Approximating an ideal filter with an FIR filter can be accomplished by using \emph{\index{window functions}{window
functions}} or \emph{\index{tapering windows}{tapering windows}}. A window function is a function that is 0 outside an interval, 
...
implementation of the Hann window function\sidenote{The Wikipedia page on Window functions \url{https://en.wikipedia.org/wiki/Window_function} contains a relatively comprehensive visual catalog of the frequency responses of different tapering window functions.}.
