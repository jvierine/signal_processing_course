...
plt.show()
\end{lstlisting}
Running the code in Listing \ref{code:ex6.6} yields the following output:
\begin{figure}[h!]
    \centering
    \includegraphics[scale=1.0]{ch06/figures/ex6.6.png}
...
\begin{itemize}
\item[a)] Listing \ref{code:ex6.7} provides a suggested solution.
\begin{lstlisting}[language=Python, caption=Modifying signals,label=code:ex6.7]
import numpy as n
...
plt.show()
\end{lstlisting}
Running Listing \ref{code:ex6.7} produces Figure \ref{fig:ex6.7}
\begin{figure}[h!]
    \centering
    \includegraphics[width=12.5cm,height=9.5cm]{ch06/figures/ex6.7.png}
    \caption{Original and modified guitar signal}
    \label{fig:ex6.7}
\end{figure}

\item[b)]
Adding the following code to Listing \ref{code:ex6.7} will save the result as an audio file.
...
\item[c)]
Based on the previous task the result of multiplying with a 2.5 Hz signal will result in the same effect, but the oscillation will be slower. After testing this by running Listing \ref{save_audio} this is exactly what happens. 
\end{itemize}
