	...
	\begin{center}
		\begin{tikzpicture}
			...
			\begin{axis}[width=1.5\textwidth, height=30em,
				...
				\addplot[domain=-100:100,samples=300] {34+
					...
					cos( deg(2*3.1415*0.01*0.5*9*x) )+
					cos( deg(2*3.1415*0.01*0.5*10*x) )+
					cos( deg(2*3.1415*0.01*0.5*11*x) )+
					cos( deg(2*3.1415*0.01*0.5*12*x) )+
					cos( deg(2*3.1415*0.01*0.5*13*x) )+
					cos( deg(2*3.1415*0.01*0.5*14*x) )+
					cos( deg(2*3.1415*0.01*0.5*15*x) ) };

				\node at (axis cs:0,30) {$=$};
				\pgfplotsinvokeforeach{24,20,...,-28}{\node at (axis cs:0,#1) {$+$};}
				\pgfplotsinvokeforeach{15,14,...,1}{
					\addplot[domain=-100:100,samples=300] {(4*#1-34)+cos( deg(2*3.1415*0.01*0.5*#1*x) ) };
				}
			\end{axis}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Spectral decomposition of a narrow pulse that consists of fifteen sinusoidal signals.}
	\label{fig:dtfig0}
\end{marginfigure}
...
Fourier decomposition of signals. By adding together the fifteen
...
fifteen sinusoidal signals. This is in essence the idea behind the
...
This may be on the minds of those for whom this is a mandatory course.
I believe that the elementary concepts taught in a basic course on this topic will provide
you with the tools to be a good data scientist and a good theoretical scientist. You will encounter the mathematical tools
...
\begin{marginfigure}[-4cm]
	\begin{center}
		\includegraphics[width=0.72\textwidth]{ch01/figures/ringach_macaque_gabor_fits.png}
	\end{center}
	\caption{Measured receptive fields of simple cells in macaque V1 (left), their two-dimensional Gabor fits (middle), and the residuals (right). Figure adapted from \citet[Fig.~2]{ringach2002spatial}.}
	\label{fig:ringach_macaque_gabor_fits}
\end{marginfigure}

\newpage

When an unknown function is expressed as a sum of harmonic basis functions,
differential operators can be applied to the individual terms. For a Fourier
basis, differentiation simply multiplies each coefficient by a frequency
factor. Spectral methods use this idea, and related global basis expansions, to
turn many differential equations into systems of algebraic equations, making
them easier to work with and often allowing efficient algorithms such as the
FFT to be used when solving the equations
\citep{boyd2001spectral}.

Frequency-selective spatial filters have been found in the mammalian
visual cortex. These neural responses select localized patterns with particular
orientations and spatial frequencies
\citep{jones1987spatial,ringach2002spatial}. In this sense, animal visual
systems have evolved mechanisms that analyze images in terms of localized
spatial-frequency content. Related filters arise in sparse
representations learned from natural images \citep{olshausen1996emergence}.
Similar orientation- and spatial-frequency-selective filters also emerge in
the first layers of deep convolutional neural networks trained to classify
images \citep{krizhevsky2012imagenet}. The same basic pattern therefore appears
in physical modeling, biological vision, and machine learning.

\begin{figure}
	\begin{center}
		\includegraphics[width=\textwidth]{ch01/figures/alexnet_first_layer_filters.png}
	\end{center}
	\caption{The 96 learned $11\times11\times3$ kernels in the first convolutional layer of
		AlexNet. Many act as oriented, spatial-frequency-selective filters, while others are
		color-opponent. Figure from \citet{krizhevsky2012imagenet}.}
	\label{fig:alexnet_first_layer_filters}
\end{figure}
...
Shannon's sampling theorem describes the information retained by sampling.
Spectral methods and frequency-selective biological and machine vision are just
a few examples of signal-processing foundations useful beyond this course.
